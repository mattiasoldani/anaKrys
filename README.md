...
anaKrys data input stage (**opening the data files** section) exploits the [succolib](https://github.com/mattiasoldani/succolib) input tools; it supports sets of formatted text files (e.g. DAT files), of NumPy compressed array files (i.e. NPZ files) and of [ROOT](https://root.cern.ch/) [tree](https://root.cern.ch/doc/master/classTTree.html) files. Multiple filesets can be opened at a time, a fileset being a set of files coming from the same data taking run. In turn, each fileset can consist of multiple files. All the filenames must share the same format, differing only in one (optionally two) part(s): the part which unambiguously identifies the fileset, i.e. the run ID and, in case of multiple files per fileset, the part that differentiates the names of the single files, usually a file ID. For example, the file list
```shell
