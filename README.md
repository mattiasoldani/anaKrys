...
The software version may vary between different applied branches. A changelog is available [here](https://sites.google.com/view/mattiasoldanianakryschangelog).
...
anaKrys data input stage (**opening the data files** section) exploits the [succolib](https://github.com/mattiasoldani/succolib) input tools; it supports sets of formatted text files (e.g. DAT files), of [NumPy](https://numpy.org/) compressed array files (i.e. [NPZ](https://numpy.org/doc/stable/reference/generated/numpy.savez.html) files) and of ROOT [tree](https://root.cern.ch/doc/master/classTTree.html) files. Multiple filesets can be opened at a time, a fileset being a set of files coming from the same data taking run. In turn, each fileset can consist of multiple files. All the filenames must share the same format, differing only in one (optionally two) part(s): the part which unambiguously identifies the fileset, i.e. the run ID and, in case of multiple files per fileset, the part that differentiates the names of the single files, usually a file ID. For example, the file list
```shell
...
Typically, data from different data taking sessions have to be dealt with separately. For each experiment two settings files must be created in ./settings with name formats
- [EXPERIMENT_NAME]_runList.py, and
- [EXPERIMENT_NAME]_settings.py

respectively. The variable `settingsFileMods` in the **input settings** section must be assigned the string `[EXPERIMENT_NAME]` for the settings to load properly. Typically, [EXPERIMENT_NAME] directly refers to the year, team and place of a specific data taking session, e.g. "y19EliotDesyT21". Template files are available with the name "y21EmptyTemplate", as well as examples from the test mode, in ./settings/test.
